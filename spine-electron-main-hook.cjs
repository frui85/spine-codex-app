...
const ELECTRON_MODULE_IDS = new Set(["electron", "electron/main"]);
...
function remoteBootstrapPrefixSource(options = {}) {
  ...
  return [
    ...
    includeLaunch ? "nohup sh -c 'exec \"$@\" </dev/null' sh " : "",
    "`",
  ].join("");
}

function remoteBootstrapLaunchSegmentSource() {
  return "` nohup sh -c 'exec \"$@\" </dev/null' sh `";
}
...
function patchRemoteBootstrapCleanupSource(source) {
  const legacyCleanupPattern =
    /` && \(pkill[^,]+`,[^,]+\(`\$\{[^}]+\}\.\*\[d\]esktop-ssh-websocket-v0\.sock`\),` \|\| true\) && nohup `/g;
  const segmentedCleanupPattern =
    /` && \(pkill[^,]+`,[^,]+\(`\$\{[^}]+\}\.\*\[d\]esktop-ssh-websocket-v0\.sock`\),` \|\| true\) && `/g;
  const legacyMatches = Array.from(String(source).matchAll(legacyCleanupPattern));
  const segmentedMatches = Array.from(
    String(source).matchAll(segmentedCleanupPattern),
  );
  if (legacyMatches.length + segmentedMatches.length !== 1) {
    throw new Error("Codex SSH app-server cleanup has an unsupported structure");
  }
  let patched;
  if (legacyMatches.length === 1) {
    patched = String(source).replace(
      legacyCleanupPattern,
      () => remoteBootstrapPrefixSource(),
    );
  } else {
    patched = String(source).replace(
      segmentedCleanupPattern,
      () => remoteBootstrapPrefixSource({ includeLaunch: false }),
    );
    const launchPattern = /` nohup `/g;
    const launchMatches = Array.from(patched.matchAll(launchPattern));
    if (launchMatches.length !== 1) {
      throw new Error("Codex SSH app-server launch has an unsupported structure");
    }
    patched = patched.replace(
      launchPattern,
      () => remoteBootstrapLaunchSegmentSource(),
    );
  }
...
function isElectronModuleUnavailable(error) {
  return (
    error?.code === "MODULE_NOT_FOUND" &&
    /^Cannot find module ['"]electron(?:\/main)?['"](?:\n|$)/.test(
...
function captureElectronMainApi(options = {}) {
  ...
  const loadInitial = options.loadInitial ?? (() => require("electron"));
  ...
  try {
    ...
    if (!isElectronModuleUnavailable(error)) throw error;
  }
...
function installMainProcessHook(options = {}) {
  ...
  try {
    if (options.rendererRecoveryOptions?.electron != null) {
      activateElectronIntegrations(options.rendererRecoveryOptions.electron);
    } else {
  ...
  deadline = setTimeout(() => {
    if (
      ...
      const missing = [
        !mainPatched && "SSH bootstrap bundle",
        !versionPatched && "version-check bundle",
        rendererRecovery == null && "Electron renderer recovery APIs",
        appServerReplayRecovery?.ready !== true && "App Server replay recovery bridge",
      ].filter(Boolean).join(", ");
      incompatible(
        `SpineCodex hook did not observe compatible ${missing} before its deadline`,
      );
    }
  }, deadlineMs);
  deadline.unref?.();
