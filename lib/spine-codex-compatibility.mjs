...
export function normalizeProductVersion(version) {
  return String(version ?? "").match(/^(\d+\.\d+\.\d+)(?:-(?:darwin|linux|win32)-(?:arm64|x64))?$/)?.[1] ?? null;
}
  ...
  for (const path of candidates) {
    try {
      ...
      if (
        ...
        normalizeProductVersion(metadata.version) != null
      ) {
        return { path, version: normalizeProductVersion(metadata.version) };
      }
    } catch {}
  }
  return null;
