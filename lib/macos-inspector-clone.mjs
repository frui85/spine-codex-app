...
export async function prepareInspectableDesktopClone({
  ...
  if (
    ...
    await isCloneInspectable(clonePath, existing.cloneIdentity, fuseIndex, tools)
  ) {
  ...
  const manifest = {
    ...
    identity,
    // Signing changes executable metadata, so record the finished clone
    // separately from its source. Desktop's updater can replace only the
    // clone while leaving the source and this manifest unchanged.
    cloneIdentity: await describeCloneIdentity(clonePath, tools),
    signing,
  };
...
async function describeCloneIdentity(clonePath, tools) {
  const identity = await describeDesktopIdentity(clonePath, tools);
  const hash = createHash("sha256");
  for await (const chunk of createReadStream(join(clonePath, "Contents", "Resources", "app.asar"))) {
    hash.update(chunk);
  }
  return { ...identity, asarSha256: hash.digest("hex") };
}

async function isCloneInspectable(clonePath, identity, fuseIndex, tools) {
  try {
    if (identity == null) return false;
    const current = await describeCloneIdentity(clonePath, tools);
    if (stableStringify(current) !== stableStringify(identity)) return false;
    await access(
