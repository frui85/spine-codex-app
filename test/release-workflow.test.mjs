...
const releaseNotes = await readFile(
  new URL("docs/RELEASE_NOTES_v26.901.51231.1.md", root),
  "utf8",
);
...
test("release tag must match the wrapper source version", () => {
  const accepted = validate("v26.901.51231.1");
  assert.equal(accepted.status, 0, accepted.stderr);
  assert.equal(accepted.stdout.trim(), "26.901.51231.1");

  const rejected = validate("v0.3.3");
  assert.notEqual(rejected.status, 0);
  assert.match(rejected.stderr, /does not match source version v26\.901\.51231\.1/);
});
...
test("active release workflow is tag-driven and publishes macOS only", () => {
  ...
  assert.match(releaseNotes, /^# SpineCodex App v26\.901\.51231\.1/m);
...
test("release notes use a tag-pinned absolute Chinese link", () => {
  const rendered = renderReleaseNotes(releaseNotes, {
    ...
    ref: "v26.901.51231.1",
    version: "26.901.51231.1",
  });
  assert.match(
    rendered,
    /\[中文发布说明\]\(https:\/\/github\.com\/frui85\/spine-codex-app\/blob\/v26\.901\.51231\.1\/docs\/RELEASE_NOTES_v26\.901\.51231\.1_ZH\.md\)/,
  );
