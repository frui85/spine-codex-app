...
- macOS 菜单默认英文，新增 English、简体中文、跟随系统选项；不支持的系统语言回退英文。
- 菜单、状态窗口、确认弹窗、辅助功能标签及复制信息随语言选择立即更新，后续启动保留偏好。
- macOS 菜单栏图标采用官方 Spine 标志。
- 新增原生 Swift 测试，覆盖语言匹配、偏好保存与状态翻译。

## v26.901.51231.1 - 2026-09-10

- 限制深层缩进，长标题最多显示两行，悬停查看完整标题，点击保留原有详情操作。
- 默认显示当前上下文投影末尾20条记录，早期记录可展开/收起；保留活跃节点、Spawn进度和选中项。
- 移除只投影前300行的截断，避免深层树当前节点被遗漏；主动展开后仍可访问早期记录。
- 原有节点/Spawn图标映射、压缩边界与操作含义保持一致。
- README补充明确标注的本地模拟数据截图，纠正外部模式轮询说明。
...
- 签名完成后记录副本自身的 Desktop 信息及 `app.asar` SHA-256，复用前核对实际内容。
- 副本变化或旧缓存缺少副本身份时，从已安装的兼容 Desktop 自动重建。
- 将本地及 fork 维护分支收拢到 `main`，保留现有兼容、回放恢复和应用列表状态切换逻辑，并纳入 SpineCodex / VS Code 集成 wiki。
- 在 Desktop `26.901.51231` 上验证重建、`Spine Tree ready` 和后续正常复用。
